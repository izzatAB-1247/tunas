<div class="clay-animate-fade-in">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
            <h2 class="fw-bold" style="color:var(--color-clay-text);"><span class="material-symbols-outlined" style="color:var(--color-clay-primary);vertical-align:middle;font-variation-settings:'FILL' 1;">workspace_premium</span> Pelatihan Persiapan Kerja</h2>
            <p style="color:var(--color-clay-text-muted);">Tingkatkan keahlianmu untuk memasuki dunia profesional.</p>
        </div>
        <div class="d-flex gap-2">
            <button class="clay-btn clay-btn-primary" data-bs-toggle="modal" data-bs-target="#addVideoModal">
                <span class="material-symbols-outlined" style="font-size:20px;">add_circle</span> Tambah Materi
            </button>
    ...
    <div class="clay-search mb-4">
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <input id="searchInput" type="text" style="border:none;background:none;width:100%;outline:none;font-size:14px;" placeholder="Cari materi...">
        </div>
    </div>

    <div class="mb-4 d-flex flex-wrap">
        <button class="clay-pill active" data-category="Semua">Semua Materi</button>
        <button class="clay-pill" data-category="Curriculum Vitae">CV & Resume</button>
        <button class="clay-pill" data-category="Interview">Teknik Wawancara</button>
        ...
        <button class="clay-pill" data-category="Etika Kerja">Etika & Budaya Kerja</button>
    </div>

    <div id="loadingState" class="text-center py-5">
        <div class="spinner-border" role="status"></div>
        <p class="mt-2" style="color:var(--color-clay-text-muted);">Memuat materi pelatihan...</p>
    </div>
    <div id="videoGrid" class="video-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:24px;margin-top:20px;"></div>
    <div id="emptyState" class="text-center py-5" style="display:none;">
        <span class="material-symbols-outlined" style="font-size:4rem;color:var(--color-clay-text-muted);">cloud_off</span>
        <h4 class="mt-3" style="color:var(--color-clay-text-muted);">Materi belum tersedia</h4>
    </div>
</div>
...
async function loadVideos() {
    try {
        const res = await fetch('{{ route("videos.index") }}?tipe=pelatihan');
    ...
    grid.innerHTML = filtered.map(v => `
        <div class="clay-video-card" onclick="playVideo('${v.youtube_id}', '${v.judul}', '${v.deskripsi || ''}', '${v.kategori}')">
            ...
            <div class="video-info">
                ...
                <h3 class="video-title" style="font-weight:700;font-size:1.1rem;margin-top:10px;color:var(--color-clay-text);">${v.judul}</h3>
                <p style="font-size:0.9rem;color:var(--color-clay-text-muted);margin:8px 0 0;">${v.deskripsi || 'Tidak ada deskripsi'}</p>
            </div>
        </div>
    `).join('');
}
...
document.getElementById('addVideoForm').addEventListener('submit', async function(e) {
    ...
    const res = await fetch('{{ route("guru.videos.store") }}', {
        ...
        body: JSON.stringify({
            url, youtube_id: vid, tipe: 'pelatihan',
    ...
    const data = await res.json();
    if (data.success) { alert('Materi berhasil ditambahkan!'); this.reset(); bootstrap.Modal.getInstance(document.getElementById('addVideoModal')).hide(); loadVideos(); }
